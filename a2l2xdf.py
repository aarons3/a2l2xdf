...
def xdf_root_with_configuration(title):
    ...
    baseoffset.set("offset", '0x' + argv[3])
...
if argv[2] == "ALL":
    # for chars in session.query(model.Characteristic).order_by(model.Characteristic.name).all():
        # #print('ch: ' + str(chars))
        # #print('ch.name: ' + str(chars.name))
        # build_table(chars, chars.name, "All", "", "", "")
    ...
    for func in session.query(model.Function).order_by(model.Function.name).all():
        ...
        for char in chars:
            ...
            #print('func.name: ' + str(func.name))            
            #print('char.name: ', str(char.name))            
            #print('char: ' + str(char))
...
#ElementTree(root).write(f"{argv[1]}.{argv[2]}.xdf")
